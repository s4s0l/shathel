...
class MvnDependencyDownloaderTest extends FeatureSpec with GivenWhenThen {
  ...
  feature("Methods tests") {
    ...
    scenario("convertFromText") {
      ...
      downloader.download(
        new StackReference("org.s4s0l.shathel.gradle.sample:simple-project:1.2.3-SNAPSHOT"),
        new File("build/MvnDependencyDownloaderTest"))
      ...
      When("Requested to download")
      downloader.download(
        new StackReference("org.s4s0l.shathel.gradle.sample2:simple-project2:DEVELOPER-SNAPSHOT"),
        new File("build/MvnDependencyDownloaderTest"))
