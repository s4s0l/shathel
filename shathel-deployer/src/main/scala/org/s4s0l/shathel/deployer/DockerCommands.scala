...
class DockerCommands(parametersCommands: ParametersCommands, environmentCommands: EnvironmentCommands)
  ...
  def docker(
    ...
    shathel(map, builder()
      ...
      val file1 = new File(storage.getTemporaryDirectory(context.environment()), "settings")
      val machineName = solution.getSolutionDescription.getName + "-" + context.environment() + "-manager-1"
      return new DockerMachineWrapper(file1).getDockerWrapperOn(machineName).getExec.executeForOutput(command)
    })
  }
